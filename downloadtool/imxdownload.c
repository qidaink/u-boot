 ...
 * File name  : imxdownload.c
 * Author     : qidaink
 * Date       : 2022-09-25
 * Version    : 
 * Description: 烧写逻裸机程序和uboot到SD卡
 * Others     : 此软件是针对NXP的IMX6U系列芯片的，软件用来烧写bin文件到SD卡里面，
 *              本软件会自动添加IVT、DCD等信息到原始的bin文件里面，主要用于裸机和uboot的烧写。
 *              gcc imxdownload.c -o imxdownload -Wall 编译即可，生成的可执行文件在ubuntu中使用 
 * 使用方法    ： 1、编译好原始的二进制bin文件，如，u-boot.bin等，并将编译好的.bin文件和本软件放置到同一个目录下！！！！
 *        	    2、执行命令 sudo ./imxdownload <soucre_bin> <sd_device>
 *                 如烧写u-boot.bin到/dev/sdc中即可使用如下所示命令:
 *                 sudo ./imxdownload u-boot.bin /dev/sdc
 * Log        : 
 * ======================================================
 */
#include "stdio.h"
